...
do
        ...
        b=$(echo $nnode | cut -d '-' -f 2)
                ...
                do       
                        #echo $i
                        echo ssh cn$j
ssh cn$j "nohup ps aux | grep afac_server | grep -v grep | awk '{print $2}' | xargs kill -9 > /dev/null 2>&1 &"
                done
        else
               #echo $nnode
		echo ssh cn$nnode
ssh cn$nnode "nohup ps aux | grep afac_server | grep -v grep | awk '{print $2}' | xargs kill -9 > /dev/null 2>&1 &"
        fi
