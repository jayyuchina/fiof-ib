...
static void establish_sock_connection_with_all_ion();
...
static int main_init()
{
    if(!init_count)
    {
        ...
        CLIENT_DEBUG_LEVEL = config_param.client_debug_level;
        ...
        if(!init_count)
        {
            ...
            {
                if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "=========== main init failed\n");
                return -1;
            }
        }
        //server_map_init();
...
{
    int srv_id;
    for(srv_id=0; srv_id<G_SERVER_NUM; srv_id++)
    {
        if(!srv_is_ion(srv_id)) continue;

        if(G_SERVER_CONNS[srv_id].sfd == 0) 	// never initialized before
        {
            G_SERVER_CONNS[srv_id].conn = cli_socket(G_SERVER_CONNS[srv_id].name, server_port, tcp_transport);
            if(G_SERVER_CONNS[srv_id].conn == NULL)
            {
                fprintf(stderr, "cli_socket failed!\n");
                return 0;
            }
            G_SERVER_CONNS[srv_id].sfd = G_SERVER_CONNS[srv_id].conn->sfd;
        }
    }
}
                                ...
                                int num_para, void **para_addr_array, uint64_t *para_len_array)
{
    ...
    if(connected != 0)
    {
        if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "ost[%d] no longer connected\n", ost_id);
        ...
        int* block_metadata, int num_block, SuccessiveInfo *successive_info)
{
    ...
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "<<< send inform_ost_to_prepare_data\n");
    ...
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, ">>> recv inform_ost_to_prepare_data\n");
}
...
// this is for read locally cached data directly
...
{
    //if(CLIENT_DEBUG_LEVEL >= 2) gettimeofday(&sema_start, NULL);
    ...
    //if(CLIENT_DEBUG_LEVEL >= 2) gettimeofday(&sema_end, NULL);
    //if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "semaphore_p:\t %lu\n", 1000000 * ( sema_end.tv_sec - sema_start.tv_sec ) + sema_end.tv_usec - sema_start.tv_usec);
}
...
static inline void device_semaphore_v()
{
    ...
    //if(CLIENT_DEBUG_LEVEL >= 2) gettimeofday(&sema_end, NULL);
    //if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "semaphore_v:\t %lu\n", 1000000 * ( sema_end.tv_sec - sema_start.tv_sec ) + sema_end.tv_usec - sema_start.tv_usec);
}
...
static inline void simulate_ssd_end()
{
	...
	//if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "simulate ssd need delay:\t %lu\n", usecs_delay);
	//usleep(usecs_delay);

	//if(CLIENT_DEBUG_LEVEL >= 2) gettimeofday(&local_delay_start, NULL);
	asm_delay(usecs_delay);
	//if(CLIENT_DEBUG_LEVEL >= 2) gettimeofday(&local_delay_end, NULL);
	...
	//if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "simulate ssd actual delay:\t %lu\n", actual_delay);
...
static void send_read_request_to_ost(int cfd, scatter_block_info* scatter_blk, int start_offset, int end_offset)
{
    ...
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "<<< send block[%d] read req to OST[%d]\n", blk_req.file_block_id, dest_ost);
...
static void send_write_request_to_ost(int cfd, scatter_block_info* scatter_blk, int offset_in_buf, int start_offset, int end_offset)
{
    ...
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "<<< send block[%d] write req to OST[%d]\n", blk_req.file_block_id, dest_ost);
}
...
static void pull_rdma_data(rdma_mem_block *mem_blk, block_data_req *blk_req, rdma_data_req *rdma_req)
{
    ...
    if(CLIENT_DEBUG_LEVEL >= 2)	gettimeofday(&pull_start_time, NULL);
    ...
    if(CLIENT_DEBUG_LEVEL >= 2)	gettimeofday(&pull_end_time, NULL);
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "~~~ rdma time: %lu\n", 1000000 * ( pull_end_time.tv_sec - pull_start_time.tv_sec ) + pull_end_time.tv_usec - pull_start_time.tv_usec);
}
...
static void rdma_init()
{
    ...
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "Init RDMA Memory: [%d Bytes]\n", ep_mem_size);
    */
...
static int srv_is_ion(int srv_id)
{
    if(srv_id % config_param.ion_modulus == 0) return 1;
    else return 0;
}
...
static void establish_ib_connection_with_all_ion()
{
    int srv_id;
    for(srv_id=0; srv_id<G_SERVER_NUM; srv_id++)
    {
        ...
        if(G_SERVER_CONNS[srv_id].sfd == 0) 	// never initialized before
        {
            ...
            if(G_SERVER_CONNS[srv_id].conn == NULL)
            {
                perror("cli_socket failed in establish_ib_connection_with_all_ion");
                exit(1);
            }
            G_SERVER_CONNS[srv_id].sfd = G_SERVER_CONNS[srv_id].conn->sfd;
        }
        ...
        // new an ib
        char tmpbuf[1024];
        G_SERVER_CONNS[srv_id].ib_ctx = create_ib_qp(tmpbuf, 1024);	// we will re-register the user memory to this ib

        // send the ib to ion
        int num_para = 2;
        void *para_addr_array[num_para];
        uint64_t para_len_array[num_para];
        para_addr_array[0] = MY_SERVER_ID;
        para_len_array[0] = sizeof(MY_SERVER_ID);
        para_addr_array[1] = &G_SERVER_CONNS[srv_id].ib_ctx->loc_ib_conn;
        para_len_array[1] = sizeof(G_SERVER_CONNS[srv_id].ib_ctx->loc_ib_conn);
        send_request(srv_id, PROTOCOL_BINARY_CMD_CREAT_IB_CONN,
                     num_para, para_addr_array, para_len_array);

    }


    // let ion new an ib too,
    // and exchange their ib info,
    for(srv_id=0; srv_id<G_SERVER_NUM; srv_id++)
    {
        if(!srv_is_ion(srv_id)) continue;

        int create_ib_ret = cli_must_read_command(G_SERVER_CONNS[srv_id].conn);
        if(create_ib_ret != 0)
        {
            fprintf(stderr, "cli_must_read_command failed in establish_ib_connection_with_all_ion()\n");
            return 0;
        }

		// the response is the IB_Connection struct
		memset(&G_SERVER_CONNS[srv_id].ib_ctx->rmt_ib_conn, 0, sizeof(G_SERVER_CONNS[srv_id].ib_ctx->rmt_ib_conn));
		assert(G_SERVER_CONNS[srv_id].conn->rbytes == sizeof(G_SERVER_CONNS[srv_id].ib_ctx->rmt_ib_conn));
        if(G_SERVER_CONNS[srv_id].conn->frag_num == 1)
        {
            memcpy(&G_SERVER_CONNS[srv_id].ib_ctx->rmt_ib_conn, G_SERVER_CONNS[srv_id].conn->rbuf, G_SERVER_CONNS[srv_id].conn->rbytes);
        }
        else if(G_SERVER_CONNS[srv_id].conn->frag_num > 1)
        {
            memcpy(&G_SERVER_CONNS[srv_id].ib_ctx->rmt_ib_conn, G_SERVER_CONNS[srv_id].conn->assemb_buf, G_SERVER_CONNS[srv_id].conn->rbytes);
        }
...
int hack_open(char *path, int flags, ...)
{
    ...
    if(CLIENT_DEBUG_LEVEL >= 2)	gettimeofday(&start_time, NULL);
    ...
    if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack open(%s)\n", absolute_path);
    ...
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "<<< send open cmd to MDS[%d]\n", server_id);
    ...
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, ">>> recv open cfd[%d]\n", ret);
    ...
    if(CLIENT_DEBUG_LEVEL >= 2) gettimeofday(&end_time, NULL);
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "~~~ open time: %lu\n", 1000000 * ( end_time.tv_sec - start_time.tv_sec ) + end_time.tv_usec - start_time.tv_usec);
...
int hack_close(int fd)
{
    if(check_is_hacked_fd(fd))
    {
        ...
        if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack close()\n");
    }
    else
    {
...
off64_t hack_lseek(int fd, off64_t offset, int whence)
{
    ...
    {
        // is cfd, go to my handle routine
        if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack lseek()\n");
    }
    else
    {
        // is real fd, which is not open by my routine, so hand it over to real_close()
...
ssize_t hack_read(int fd, void *buf, size_t count)
{
    ...
    {
        // is cfd, go to my handle routine
        if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack read()\n");
    }
    else
    {
    ...
    if(CLIENT_DEBUG_LEVEL >= 2)	gettimeofday(&sndreq_start_time, NULL);
    ...
    if(CLIENT_DEBUG_LEVEL >= 2)	gettimeofday(&sndreq_end_time, NULL);
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "~~~ sndreq time: %lu\n", 1000000 * ( sndreq_end_time.tv_sec - sndreq_start_time.tv_sec ) + sndreq_end_time.tv_usec - sndreq_start_time.tv_usec);
    ...
    if(CLIENT_DEBUG_LEVEL >= 2) gettimeofday(&epoll_start_time, NULL);


    struct timeval wait_start_time, wait_end_time;
    if(CLIENT_DEBUG_LEVEL >= 2) gettimeofday(&wait_start_time, NULL);
    ...
    while(1)
    {
        ...
        for(cur_evt=0; cur_evt<epoll_ret; cur_evt++)
        {
            ...
            if(events[cur_evt].events & EPOLLIN)
            {
                ...
                if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "conn read %d bytes\n", G_SERVER_CONNS[cur_ost].conn->rbytes);
                ...
                while(handled_bytes < tmp_rbuf_len)
                {
                    ...
                    if((ost_resp->response.magic == PROTOCOL_BINARY_RES) && (ost_resp->response.opcode == PROTOCOL_BINARY_CMD_OST_PATH_REQ))
                    {
                        ...
                        if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, ">>> send PATH for block[%d] to OST[%d]\n", re_blk_req->file_block_id, cur_ost);
                    ...
                    else if((ost_resp->response.magic == PROTOCOL_BINARY_RES) && (ost_resp->response.opcode == PROTOCOL_BINARY_CMD_OST_NO_FREE_MEM_BLK))
                    {
                        ...
                        if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, ">>> RESEND req of block[%d] to OST[%d]\n", re_blk_req->file_block_id, cur_ost);
                    ...
                    else
                    {
                        ...
                        if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, ">>> read %d bytes of block[%d] from OST[%d]\n", recv_rdma_req->len, recv_blk_req->file_block_id, cur_ost);
    ...
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "~~~ total wait time: %lu\n", total_wait_time);

    if(CLIENT_DEBUG_LEVEL >= 2) gettimeofday(&epoll_end_time, NULL);
    if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "~~~ ost time: %lu\n", 1000000 * ( epoll_end_time.tv_sec - epoll_start_time.tv_sec ) + epoll_end_time.tv_usec - epoll_start_time.tv_usec);
...
ssize_t hack_write(int fd, const void *buf, size_t count)
{
    ...
    {
        // is cfd, go to my handle routine
        if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack write()\n");
    }
    else
    {
        // is real fd, which is not open by my routine, so hand it over to real_func()
    ...
    while(1)
    {
        ...
        //if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "epoll_wait get %d events\n", epoll_ret);
        int cur_evt;
        for(cur_evt=0; cur_evt<epoll_ret; cur_evt++)
        {
            if ((events[cur_evt].events & EPOLLERR) || (events[cur_evt].events & EPOLLHUP) || (!(events[cur_evt].events & EPOLLIN)))
            ...
            if(events[cur_evt].events & EPOLLIN)
            {
                //if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, "OST[%d] EPOLLIN\n", events[cur_evt].data.u32);
                ...
                if(CLIENT_DEBUG_LEVEL >= 3) fprintf(stderr, "conn read %d bytes\n", G_SERVER_CONNS[cur_ost].conn->rbytes);
                ...
                while(handled_bytes < tmp_rbuf_len)
                {
                    ...
                    else
                    {
                        ...
                        if(CLIENT_DEBUG_LEVEL >= 2) fprintf(stderr, ">>> write %d bytes of block[%d] to OST[%d]\n", write_len, recv_file_blk_id, cur_ost);
...
FILE * hack_fopen(const char *path, const char *mode)
{
    ...
    // hack
    if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack fopen(%s)\n", path);
...
int hack_fclose(FILE *stream)
{
    ...
    if(check_is_hacked_fd(fd))
    {
        ...
        if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack fclose()\n");
...
int hack_fseek(FILE *stream, long offset, int whence)
{
    ...
    {
        // the FILE* stream is not linked to a real fd, which means this is a hacked cfd
        if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack fseek()\n");
...
size_t hack_fread(void *buffer, size_t size, size_t count, FILE *stream)
{
    ...
    {
        // the FILE* stream is not linked to a real fd, which means this is a hacked cfd
        if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack fread()\n");
...
{
    if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack open64()\n");
...
{
    if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack lseek64()\n");
...
size_t DARSHAN_DECL(fwrite)(const void *ptr, size_t size, size_t nmemb, FILE *stream)
{
    ...
    {
        // the FILE* stream is not linked to a real fd, which means this is a hacked cfd
        if(CLIENT_DEBUG_LEVEL) fprintf(stderr, "### hack fwrite()\n");
